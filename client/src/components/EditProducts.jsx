...
function EditProducts({ product, closeEditProdouct }) {
  const { loading, page, limit} = useSelector((state) => state.product);
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setValue("categoryId", product.categoryId[0]._id);
      setValue("sub_categoryId", product.sub_categoryId[0]._id);
      ...
      if (product.more_details) {
        const fields = Object.entries(product.more_details).map(([key, value]) => ({
          key,
          value
        }));
  ...
  return (
    <section className="fixed inset-0 flex justify-center bg-black/50 backdrop-blur-sm z-50 p-3 overflow-y-auto no-scrollbar h-full">
      <div className="bg-white h-fit w-full max-w-3xl p-4 rounded-lg shadow-lg">
        <div className="flex justify-between items-center pb-2">
          <h2 className="font-semibold"> Edit Product</h2>
          <button onClick={closeEditProdouct} className="cursor-pointer hover:text-primary-200">
